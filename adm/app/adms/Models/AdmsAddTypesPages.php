...
class AdmsAddTypesPages
{
    ...
    private function viewlastTypesPages()
    {
        ...
        $viewlastTypesPages->fullRead("SELECT id, name, order_type_pg FROM adms_type_pgs ORDER BY order_type_pg DESC LIMIT 1");
        $this->resultQuery = $viewlastTypesPages->getResult();
        if($this->resultQuery){
            $this->data['order_type_pg'] = $this->resultQuery[0]['order_type_pg'] + 1;
            return true;
        }
    }
